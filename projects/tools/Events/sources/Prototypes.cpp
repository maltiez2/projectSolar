...
namespace projectSolar::Prototypes
{
	...
	TestEventHandler::TestEventHandler(const size_t& workersNumber, const size_t& bufferSize) :
	...
	void TestEventHandler::processEvent(uint8_t eventType, uint8_t* data)
	{
		switch (eventType)
		{
			EVENTS_DEF_UNKNOWN();
			...
			EVENT_DEF(DEBUG_EVENT);
			{
				TestEventHandler* destination = this;
				char message[128];
				for (uint8_t i = 0; i < eventData.size; i++)
				{
					message[i] = eventData.message[i];
				}

				SEND_EVENT(TestEventHandler::TEST_MSG, destination, eventData.size, *message);
			}
			EVENT_DEF(TEST_MSG);
			{
				std::string message = eventData.message;
				message.erase(eventData.size, std::string::npos);
				LOG_DEBUG("[TEST_MSG] ", message);
			}
			EVENT_DEF(EMPTY_DATA_EVENT);
			{
				//
			}
			EVENTS_DEF_DEFAULT()
